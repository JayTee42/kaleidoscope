...
namespace Kaleidoscope.Ast
{
	...
	{
		// The module we generate code into
		...
		// Create a new code generator with the given module to generate code into.
		...
		public void VisitConditionalExpression(ConditionalExpression expr)
		{
			...
			// It might have changed, e. g. by another condition.
			...
			// Fill the "merge" block with a phi node.
		...
		public void VisitFunctionPrototype(FunctionPrototype prototype)
		{
			...
			if (func.Pointer != IntPtr.Zero)
			{
				...
				// Make sure that the number of parameters is consistent.
			...
			{
				// The prototype does not exist yet, we have to create it.
				// Make the function type which is always (double, double, ...) -> double.
				...
				// Assign our names to the parameters (only for nice IR, we never query them again).
		...
		public void VisitFunctionDefinition(FunctionDefinition definition)
		{
			...
			// Now we have to bring the parameters into scope.
			// Walk the parameters of the function, but use the names from the definition.
			// Those might vary (aka the tutorial's "[...] see what you can come up with" comment).
			// I am pretty sure this solves the problem in a very elegant manner :)
			...
			for (int i = 0; i < definition.Prototype.ParameterNames.Count; i++)
			{
				...
				// Note: The `FunctionPrototype` class ensures that its parameters have unique names.
			...
			// Ask the module to optimize the function.
			// If optimization is disabled, this is a no-op.
